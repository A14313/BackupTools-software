...
echo 2 Copy files
echo 3 Move files
echo 4 Delete "File too long" error or delete all files from the directory%NL%%NL%%NL%
echo 8 Want to buy me some coffee?
echo 9 CREDITS
echo 0 to QUIT%NL%%NL%%NL%
...
set switch=/MOVE
set word=MOVE
set phrase1=This will MOVE the files from source folder to your destination folder and OVERWRITE the files.
set phrase2=It will DELETE the files from source folder after moving all files to the destination folder.
set phrase3=This might delete some files if you typed the source or destination incorrect.
...
:SRC
cls
set /p src=Where is your source folder? The folder that you want to %word%%NL%::
IF NOT EXIST "%src%" (
color c
...
:DEST
set /p dest=Where is your destination folder?%NL%::
IF NOT EXIST "%dest%" (
color c
...
echo %phrase2%
echo.
echo Please review the source folder and destination folder before proceeding.
echo %phrase3%
echo.
echo %phrase4%
echo %phrase4_1%
...
color a
cls
set /p question1=Do want to input the source folder again?%NL%%NL%Press Y for yes and N for no%NL%M to go back to main menu%NL%::
goto %question1%
...
color a
cls
set /p question2=Do want to input the destination folder again?%NL%%NL%Press Y for yes and N for no%NL%M to go back to main menu%NL%::
goto %question2%
...
:SRC2
cls
set /p src2=Where is your source folder? The folder that you want to copy%NL%::
IF NOT EXIST "%src2%" (
color c
echo The source directory does not exist
pause
...
:DEST2
set /p dest2=Where is your destination folder?%NL%::
IF NOT EXIST "%dest2%" (
color c
echo The destination directory does not exist
pause
...
:FILETOOLONG
cls
set /p toolongfolder=Where is the directory or folder that has "file too long" error%NL%or the directory or folder that you want to delete?%NL%::
IF NOT EXIST "%toolongfolder%" (
color c
echo The directory or folder does not exist
pause
goto QUESTION1
) ELSE (
goto CONFIRMATION)
...
echo This will delete all the files and folders from the directory or folder that you typed.
echo.
...
color a
cls
set /p question1=Do want to input the directory or folder again?%NL%%NL%Press Y for yes and N for no%NL%M to go back to main menu%NL%::
goto %question1%
